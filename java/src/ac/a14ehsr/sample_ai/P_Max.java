...
public class P_Max {
    ...
    private int[] weight;
    private Scanner sc;
    static final String playerName = "P_Max";
    ...
    private int select(int[][][][] record, int game, int sequenceNumber) {
        ...
        for (int i = 0; i < record[game][sequenceNumber].length; i++) {
            if (record[game][sequenceNumber][i][0] == -1) {
                selectableNode.add(new NumPair(i, weight[i]));
            }
        }
        int max = selectableNode.stream().mapToInt(np -> np.num).max().getAsInt();
        List<NumPair> maxList = selectableNode.stream().filter(np -> np.num == max)
